static const char *fragment_shader_anop =
    ...
    "uniform int render_type;    // 0: SDR, 1: HDR10, 2: HLG, 3: DoviP5, 4: VLC_HDR10, 5 VLC_HLG, 6: Anop_HDR10, 7: Anop_HLG\n"
    "\n"
    "// Adjustment\n"
    "vec4 adjustment(vec4 color, float brightness, float contrast, float saturation) {\n"
    "    // brightness\n"
    ...
    "\n"
    "// ANOP\n"
    "vec4 anop_hdr10_to_sdr(vec4 color) {\n"
    ...
    "    fragColor = mix(vec3(gray), fragColor, 1.2);\n"
    "    return vec4(fragColor, 1.0);\n"
    "}\n"
    "\n"
    "vec4 anop_hlg_to_sdr(vec4 color) {\n"
    "    vec3 fragColor = color.rgb;\n"
    "    const mat3 rgb2xyz2020 = mat3(vec3(0.6370, 0.1446, 0.1689),\n"
    "                                  vec3(0.2627, 0.6780, 0.0593),\n"
    "                                  vec3(0.0000, 0.0281, 1.0610));\n"
    ...
    "        color = anop_hdr10_to_sdr(color);\n"
    "    } else if (render_type == 7) {\n"
    "        color = anop_hlg_to_sdr(color);\n"
    "    }\n"
    "    gl_FragColor = color;\n"
    "}\n"
;
